...
function EnrollAdmin() {
    ...
    function submitHandle(e) {
        ...
        if (go) {
            ...
            if (!mobPattern.test(mobile)) {
                ...
                var d = window.confirm(" Enroll Admin")
                ...
                if (d === true) {
                    ...
                    let c = Cookies.get("Authorization")
                    console.log(c);
                    Axios.post(process.env.REACT_APP_SECURITY_API.concat("adminenroll"), formdata, { headers: { Authorization: Cookies.get("Authorization") } }).then((res) => {
                        console.log(res);
                        alert(res.data.msg)
                        setSpin(false)
    ...
    return (
        <div >
            <p style={{ fontSize: 30, fontWeight: 'bolder', fontFamily: 'initial', color: '#001529', textAlign: 'center' }}>Admin Enrollment Form</p><hr />
            {spin ? <div style={{ textAlign: 'center' }}><Spin /> </div> : <form style={{ marginTop: '3%' }} onSubmit={submitHandle}>
                ...
                <Form.Row>
                    <Form.Group as={Col} controlId="formGridCity">
                        ...
                        <input className="form-control" onChange={(e) => setSchoolCode(e.target.value)} required type="text" placeholder="Enter Code" />
